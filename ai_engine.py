...
{{
    ...
    def generate_blog_content(self, client_data, keyword, style, keyword_average, additional_notes="", intent="Informativo", kw_type="Short tail", content_format="Blog post content", internal_links=None):
        ...
        10. Citação de Terceiros (REGRA DE OURO): Sempre que usar informações, dados ou estatísticas de fontes externas (3rd party), você DEVE embutir o link da fonte diretamente em uma das palavras-chave relevantes da própria frase. É ABSOLUTAMENTE PROIBIDO citar apenas o domínio (ex: 'google.com', 'wikipedia.org'). Você DEVE usar a URL profunda e final (longtail) da página exata de onde a informação foi extraída (ex: 'https://site.com/subpasta/artigo-detalhado'). Se não souber a URL exata, pesquise mentalmente ou use seu conhecimento para reconstruir o caminho mais provável da página de conteúdo. Links genéricos para homepages serão considerados falha técnica grave.
        11. Linkagem Interna: A mesma regra da URL completa (longtail) vale para links internos. Se for linkar um curso, serviço ou página específica dentro do domínio do cliente ({client_data['url']}), você DEVE achar o link exato e relevante. NÃO aponte links internos para a Home se houver uma página interna mais específica. Use as URLs fornecidas no contexto de linkagem interna com precisão cirúrgica.
